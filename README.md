...
<p align="center">
  <img src="docs/logo.png" alt="KengaOS" width="760"/>
</p>
...
Операционная система нового поколения, написанная на языке <a href="https://github.com/GermannM3/kenga-lang">Kenga</a> и загружаемая через <a href="https://limine-bootloader.org">Limine</a>. Планировщик, IPC, интерактивный shell — всё на собственном ядре.
...
| **Процессы + IPC** (`k_proc_spawn`, `k_ipc_send`/`k_ipc_recv`, per-proc message queues) | ✅ |
...
- **Прерывный планировщик** (сейчас кооперативный)
- **Физическая память: page allocator → heap → buddy**
- **Paging + address spaces + syscalls**
- **init + kenga-agent через IPC**
