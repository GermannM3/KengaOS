...
## Готовый ISO (без сборки)

Скачай `kengaos.iso` со страницы [Releases](../../releases), запиши на
флешку (Rufus/Etcher) и загрузись с неё (Secure Boot — выключить).
Живой рабочий стол: терминал, агенты с IPC, «Магазин» пакетов
(`install a` в input-баре), ring-3 пользовательские ELF-программы.
Статус поддержки железа — [docs/HARDWARE.md](docs/HARDWARE.md).
...
> **Честное сравнение с Windows / Linux / Android / iOS** — [`docs/COMPARISON.md`](docs/COMPARISON.md):
> по инфраструктуре мы прототип; наша ставка — системный язык для ИИ, пророки в ядре и
> локальное обучение.
