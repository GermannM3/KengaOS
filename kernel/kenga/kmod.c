...
int64_t k_kmod_selftest(void);

int64_t k_kmod_selftest(void) {
  int64_t k_magic = 90;
  __asm__ __volatile__("outb %0, %1" : : "a"((uint8_t)(0)), "Nd"((uint16_t)(112)));
  (void)(0);
  uint8_t _k_port_in;
  __asm__ __volatile__("inb %1, %0" : "=a"(_k_port_in) : "Nd"((uint16_t)(113)));
  int64_t k_sec = ((int64_t)_k_port_in);
  int64_t k_packed = ((k_magic << 8) | k_sec);
  return k_packed;
}

int64_t main(void) {
  return k_kmod_selftest();
}
