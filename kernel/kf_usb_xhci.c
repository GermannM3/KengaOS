/* kf_usb_xhci.c — xHCI host controller + HID tablet (QEMU usb-tablet и
   реальные USB-мыши/клавиатуры). Один и тот же MMIO/ring-код для x86_64 и
   aarch64 (порт usb_a64.c). Отличия только в PCI-доступе:
   x86 — порты 0xCF8/0xCFC, MMIO — phys+hhdm.
...
/* --- PCI config через порты 0xCF8/0xCFC (x86) --- */
static inline uint8_t  x_inb(uint16_t p)  { uint8_t v;  __asm__ __volatile__("inb %1,%0" : "=a"(v) : "Nd"(p)); return v; }
static inline void     x_outb(uint16_t p, uint8_t v) { __asm__ __volatile__("outb %0,%1" : : "a"(v), "Nd"(p)); }
static inline uint32_t x_inl(uint16_t p)  { uint32_t v; __asm__ __volatile__("inl %1,%0" : "=a"(v) : "Nd"(p)); return v; }
static inline void     x_outl(uint16_t p, uint32_t v) { __asm__ __volatile__("outl %0,%1" : : "a"(v), "Nd"(p)); }

static void lg_uart(const char* s) { for (; *s; s++) x_outb(0x3F8, (uint8_t)*s); }
...
static void ulog(const char* s) { lg_uart(s); }
...
int64_t k_xhci_init(void) {
    ...
    bar += (uint64_t)k_kf_get_hhdm();   /* MMIO через HHDM (x86) */
...
int64_t k_xhci_ready(void) { return usb_ok; }
static int64_t k_xhci_tab_present(void) { return tab_present; }
int64_t k_xhci_tab_x(void) { return tab_x; }
int64_t k_xhci_tab_y(void) { return tab_y; }
int64_t k_xhci_tab_btn(void) { return tab_btn; }
